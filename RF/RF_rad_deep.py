...
for model_name in deep.keys():
    ...
    for index,split in enumerate(kfolds):
        ...
        path="results/RF_rad_deep/"+model_name+"_RF_nsplit"+str(index)
        os.mkdir(path)
        ...
        for key in list(selected_deep.keys()):
            try:
                combined_patterns_rad_deep[key] = np.concatenate((selected_radiomics[key], selected_deep[key]))
                #print(combined_patterns)
            except:
                print(key)
                continue 
        #print(combined_patterns_rad_deep)
        ...
        plt.grid(True)
        plt.savefig(path+"/plot_performance_model for" + model_name + "and nsplit" + str(index) + ".png",dpi=300)
        plt.clf()
        ...
        plt.title("CI %.4f" % score_)
        plt.grid(True)
        plt.savefig(path+"/plot_performance_hazard_model for" + model_name + "and nsplit" + str(index) + ".png",dpi=300)
        plt.clf()
    ...
    results["rad_deep for model "+model_name] = pd.Series({"Maximum CI":max(Concordance_index),"Minimum CI":min(Concordance_index),"Overall CI":statistics.mean(Concordance_index),"Standard Deviation":statistics.stdev(Concordance_index)})    
